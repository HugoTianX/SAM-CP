...
from .BertForMLM4Contra import BertForMaskedLM
            ...
            if token_idxs is not None and token_idxs != []:
                ...
                # labels_ids_tmp[start_idx + 1] = torch.tensor([labels_ids_tmp[start_idx + 1]] + [tokenizer.encode(syn, add_special_tokens=False)[0] for syn in syns])
                    if random.random() < 0.5:
                        inputs.append([input_ids_tmp, input_ids_tmp])
                        labels.append([labels_ids_tmp, labels_ids_tmp])               
                        attention_masks.append([[1]*len(input_ids_tmp), [1]*len(input_ids_tmp)])
...
# Load JSON data
data = json.load(open(json_file, 'r'))  # Replace 'json_file' with your variable
